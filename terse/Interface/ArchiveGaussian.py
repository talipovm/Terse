...
if __name__ == "__main__":
    ...
    append_path = os.path.abspath(sys.argv[0])[:-26]
    print("Append to PYTHONPATH: %s" % (append_path))
    sys.path.append(append_path)
...
class ArchiveGauStep(ElectronicStructure):
    ...
    def parse(self):
        ...
        """
        Occur only in the first entrance, so we just safely skip_n
        ...
        s18_geom                     = self.FI.find_text_block(start_offset=1)
        s_ag_variables               = self.FI.find_text_block(start_offset=1) # after geometry
