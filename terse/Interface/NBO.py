...
Several kinds of syntax will be available for dictionaries (on example of NBO):
    ...
    2. Normal, e.g. --inbo c=a.chk --inbo l=a.log
    3. Extended, e.g. --inbo c1=a.chk --inbo l1=a.log --inbo c2=b.chk --inbo l2=b.log
...
class nbo_result(Top):
    ...
    def atomByID(self,i):
        atnames = list(map(lambda k: k.idx, self.atoms))
        ati = atnames.index(i.strip())
        return self.atoms[ati]

    def nboByID(self,i):
        nbonames = list(map(lambda k: k.idx, self.orbs))
    ...
    def read_atomic_info(self,FI):
        ...
        d = FI.find_text_block(start_match='Summary of Natural Population Analysis', start_offset=6, end_match='===')
        for s in d:
            ss = s.split()
            if ss[0].lower()=='bq':
...
class NBO(Top):
    ...
    def parse(self):
        ...
        for rSet in setnames:
            ...
            d = FI.find_text_block(start_match='NHO Directionality and "Bond Bending"', start_offset= 10)
            for s in d:
            ...
            #d = FI.find_text_block(
                    ...
                    #)
            for s in d:
                rs = re.search('^\D*(\d+)\..*?\/(\d+)\..*?(\S+)\s+(\S+)\s+(\S+)$',s)
                if not rs:
                    continue
                sopta = nbo_sopta()
                sopta.parse(rs,nr)
        ...
        if self.OpenShell:
            for rSet in ('setA','setB'):
                ...
                for orb in nr.orbs:
                    ...
                    self.topology.increase_order(s_at1, s_at2)
                    self.topology.increase_order(s_at2, s_at1)

            chargesA = list(map(lambda a: float(a.charge), self.setA.atoms))
            chargesB = list(map(lambda a: float(a.charge), self.setB.atoms))
            for i in range(len(chargesA)):
                self.charges.data.append(chargesA[i] + chargesB[i])
                self.spins.data.append(  chargesB[i] - chargesA[i])  
            ...
            for orb in self.setAB.orbs:
                ...
                self.topology.increase_order(s_at1, s_at2)
                self.topology.increase_order(s_at2, s_at1)
            self.charges.data = list(map(lambda a: float(a.charge), self.setAB.atoms))
        return
...
if s_nbo_bond:
    ...
    nbo_top.increase_order(at1, at2)
    nbo_top.increase_order(at2, at1)
"""
